...
PORT = '8000'# 0 1
DATA_ORDER = 'rel'
# DATA_ORDER = 'rel' if PORT == '8001' else 'abs'
SAMPLE = 60
CHUNK = 60
init_state_id = 2
...
TASK_PATH = {
    "adjust bottle": "ajust_bottle_0819",
    "dump bin bigbin": "dump_bin_bigbin_0819",
    "grab roller": "grab_roller_0819",
    "lift pot": "lift_pot_new_0823",
    "move playingcard away": "move_playingcard_away_0820",
    "open laptop": "open_laptop_0822",
    "place burger fries": "place_burger_fries_0821",
    "place container plate": "place_container_plate_0822",
    "press stapler": "press_stapler_0822",
    
    "shake bottle": "shake_bottle_0823",
    "shake bottle horizontally": "shake_bottle_0823"
}
...
def upsample_action(action, new_len):
    ...
    """
    # task2
    # action[:, 9] = np.where(action[:, 9] > 0.02, 0.03, action[:, 9])
    # action[:, 9] = np.where(action[:, 9] <= 0.02, -0.002, action[:, 9])

    # task4
    action[:, 9] = np.where(action[:, 9] < 0.03, -0.002, action[:, 9])
    action[:, 19] = np.where(action[:, 19] <= 0.01, -0.002, action[:, 19])
...
def model_inference(args, config, ros_operator, save_episode=True):
    ...
    input("Enter any key to continue ee:")
