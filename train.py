...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for data in train_dataloader:
            past_time = time.time()
            model.train()
            inputs = {**{key: value.cuda(non_blocking=True) for key, value in data.items()},}
            optim.zero_grad()
            loss = model(**inputs)
            ...
            if iters % args.log_interval == 0: 
                ...
                accelerator.print(f"[epoch] {epoch} [Iter {iters}] [Training Loss] {loss.item()} [time_per_iter] {time.time() - past_time}")
            iters += 1
            
            
        if epoch % args.save_interval == 0 and epoch != 0:
            accelerator.print("========start saving models=========")
            accelerator.save_state(os.path.join(output_dir, f"ckpt-{iters}"), safe_serialization=True)
            
        accelerator.wait_for_everyone()
    
    model.eval()
    accelerator.print("========start saving models=========")
    accelerator.save_state(os.path.join(output_dir, f"ckpt-final"), safe_serialization=True)
