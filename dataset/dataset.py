    ...
    def __init__(self,
        ...
        for file in meta_files:
            with io.BytesIO(fileio.get(fileio.join_path(metas_path, file))) as f:
                ...
                #         else:
                #             self.database[data_path] = {'images/top_image': file['images/top_image'][:],
                #                                         'images/wrist_image': file['images/wrist_image'][:],
                #                                         'qpos': file['qpos'][:],
                #                                         'eef_6d': file['eef_6d'][:]
                #                                         }
                #             self.datalist.extend([data_path, idx, meta['task_name']] for idx in range(len_data - 2))
                for data_path in meta['datalist'][:sample_num]:
                    ...
                    with h5py.File(io.BytesIO(fileio.get(data_path)), 'r') as file:
                        ...
                        if "observations" in file.keys():
                            ...
                            self.database[data_path] = {
                                'observations/images/cam_high': file['observations/images/cam_high'][:],
                                'observations/images/cam_left_wrist': file['observations/images/cam_left_wrist'][:],
                                'observations/qpos': file['observations/qpos'][:],
                                'observations/eef_6d': file['observations/eef_6d'][:]
                            }

                            len_data = file['observations/qpos'].shape[0]

                            self.datalist.extend(
                                [data_path, idx, meta['task_name']]
                                for idx in range(len_data - 5)
                            )

                        else:

                            self.database[data_path] = {
                                'images/top_image': file['images/top_image'][:],
                                'images/wrist_image': file['images/wrist_image'][:],
                                'qpos': file['qpos'][:],
                                'eef_6d': file['eef_6d'][:]
                            }

                            len_data = file['qpos'].shape[0]

                            self.datalist.extend(
                                [data_path, idx, meta['task_name']]
                                for idx in range(len_data - 2)
                            )
        ...
        self.language_emb = torch.load("assets/encoded_language.pt", map_location="cpu")
    
    def read_hdf5(self, datapath, idx, task_name):
        data = self.database[datapath]
        
        
        if 'observations/images/cam_high' in data.keys():
            current_abs_joint = data['observations/qpos'][:]
            current_abs_eef6d = data['observations/eef_6d'][:]
            images = [data[key] for key in ['observations/images/cam_high', 'observations/images/cam_left_wrist']]
            image_input =  torch.stack([self.image_aug(decode_image_from_bytes(img[idx])) for img in images])
            query_duration = 2
            freq = 30
        else:
            current_abs_joint = data['qpos'][:]
            current_abs_joint = np.concatenate([current_abs_joint, np.zeros_like(current_abs_joint)], axis = -1)
            current_abs_eef6d = data['eef_6d'][:]
            current_abs_eef6d = np.concatenate([current_abs_eef6d, np.zeros_like(current_abs_eef6d)], axis = -1)
            images = [data[key] for key in ['images/top_image', 'images/wrist_image']]
            image_input =  torch.stack([self.image_aug(decode_image_from_bytes(img[idx])) for img in images])
            query_duration = 1
            freq = 30
        ...
        our_rel_eef = current_abs_eef6d[1:] - current_abs_eef6d[:1]
        our_rel_eef[:, 3:10] = current_abs_eef6d[1:, 3:10]
        our_rel_eef[:, 13:20] = current_abs_eef6d[1:, 13:20]
        
        bad_rel_joint = current_abs_joint[1:] - current_abs_joint[:-1]
        bad_rel_joint[:, 6] = current_abs_joint[1:, 6]
        bad_rel_joint[:, 13] = current_abs_joint[1:, 13]    
        
        bad_rel_ee = current_abs_eef6d[1:] - current_abs_eef6d[:-1]
        bad_rel_ee[:, 9] = current_abs_eef6d[1:, 9]
        bad_rel_ee[:, 19] = current_abs_eef6d[1:, 19]
        ...
        return {
            ...
            # 'our_rel_eef_action': torch.tensor(our_rel_eef).to(torch.float32),
            # 'bad_rel_joint_action': torch.tensor(bad_rel_joint).to(torch.float32),
            # 'bad_rel_eef_action': torch.tensor(bad_rel_ee).to(torch.float32)
        }
