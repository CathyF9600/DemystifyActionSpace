...
torchrun --nproc-per-node=2 --nnodes=1 --node-rank=0 --master-addr=localhost --master-port=$port train.py \
  ...
  --precision bf16 \
  --port $port \
  --output_dir runnings/RoboTwin/abs_ee_flow_adjust_new \
  --wandb_name robotwin2_abs_ee_flow_adjust_new \
  --metas_path /home/fyc/EmpiricalStudyForVLA/datasets/meta_files/robotwin2_abs_ee_adjust.jsonl  
