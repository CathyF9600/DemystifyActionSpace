...
torchrun --nproc-per-node=2 --nnodes=1 --node-rank=0 --master-addr=localhost --master-port=$port train.py \
  ...
  --precision bf16 \
  --port $port \
  --output_dir runnings/RoboTwin/rel_qpos_flow \
  --wandb_name robotwin2_rel_qpos_flow \
  --metas_path /home/fyc/EmpiricalStudyForVLA/datasets/meta_files/robotwin2_rel_qpos.jsonl  
